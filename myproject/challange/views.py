...
from .models import ChallangeContent, ChallangePostViewTracking
...
def render_challange_post(request, slug):
    post = get_object_or_404(ChallangeContent, slug=slug)
    if request.user.is_authenticated:
        ChallangePostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'challange/challange_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = ChallangeContent.objects.all()
    return render(request, 'challange/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_challange_post(request):
    if request.method == "POST":
        ...
        new_post = ChallangeContent.objects.create(
            title=title,
            summary=summary,
