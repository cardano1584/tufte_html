...
from .models import DefinitionsContent, DefinitionsPostVersion

class PostVersionInline(admin.TabularInline):
    model = DefinitionsPostVersion
    ...
    class Media:
        js = (
            '/static/definitions/codemirror/lib/codemirror.js', 
            '/static/definitions/codemirror/mode/markdown/markdown.js', 
            '/static/definitions/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/definitions/codemirror/lib/codemirror.css',
            )
        }
...
class DefinitionsContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            DefinitionsPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(DefinitionsPostVersion, pk=version_id)
...
admin.site.register(DefinitionsContent, DefinitionsContentAdmin)
