...
from .models import TicketContent, TicketPostVersion

class PostVersionInline(admin.TabularInline):
    model = TicketPostVersion
    ...
    class Media:
        js = (
            '/static/ticket/codemirror/lib/codemirror.js', 
            '/static/ticket/codemirror/mode/markdown/markdown.js', 
            '/static/ticket/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/ticket/codemirror/lib/codemirror.css',
            )
        }
...
class TicketContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            TicketPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(TicketPostVersion, pk=version_id)
