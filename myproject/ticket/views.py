...
from .models import TicketContent, TicketPostViewTracking
...
def render_ticket_post(request, slug):
    post = get_object_or_404(TicketContent, slug=slug)
    if request.user.is_authenticated:
        TicketPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'ticket/ticket_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = TicketContent.objects.all()
    return render(request, 'ticket/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_ticket_post(request):
    if request.method == "POST":
        ...
        new_post = TicketContent.objects.create(
            title=title,
            summary=summary,
