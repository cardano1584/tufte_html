...
from .models import ChallangesContent, ChallangesPostViewTracking
...
def render_challanges_post(request, slug):
    post = get_object_or_404(ChallangesContent, slug=slug)
    if request.user.is_authenticated:
        ChallangesPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'challanges/challanges_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = ChallangesContent.objects.all()
    return render(request, 'challanges/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_challanges_post(request):
    if request.method == "POST":
        ...
        new_post = ChallangesContent.objects.create(
            title=title,
            summary=summary,
