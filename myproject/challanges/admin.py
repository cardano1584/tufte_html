...
from .models import ChallangesContent, ChallangesPostVersion

class PostVersionInline(admin.TabularInline):
    model = ChallangesPostVersion
    ...
    class Media:
        js = (
            '/static/challanges/codemirror/lib/codemirror.js', 
            '/static/challanges/codemirror/mode/markdown/markdown.js', 
            '/static/challanges/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/challanges/codemirror/lib/codemirror.css',
            )
        }
...
class ChallangesContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            ChallangesPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(ChallangesPostVersion, pk=version_id)
...
admin.site.register(ChallangesContent, ChallangesContentAdmin)
