...
from .models import TransformationContent, TransformationPostViewTracking
...
def render_transformation_post(request, slug):
    post = get_object_or_404(TransformationContent, slug=slug)
    if request.user.is_authenticated:
        TransformationPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'transformation/transformation_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = TransformationContent.objects.all()
    return render(request, 'transformation/sql_definitions.html', {'all_posts': all_posts})
...
def create_transformation_post(request):
    if request.method == "POST":
        ...
        new_post = TransformationContent.objects.create(
            title=title,
            summary=summary,
