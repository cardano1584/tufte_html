...
from .models import TransformationContent, TransformationPostVersion

class PostVersionInline(admin.TabularInline):
    model = TransformationPostVersion
    ...
    class Media:
        js = (
            '/static/transformation/codemirror/lib/codemirror.js', 
            '/static/transformation/codemirror/mode/markdown/markdown.js', 
            '/static/transformation/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/transformation/codemirror/lib/codemirror.css',
            )
        }
...
class TransformationContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            TransformationPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(TransformationPostVersion, pk=version_id)
...
admin.site.register(TransformationContent, TransformationContentAdmin)
