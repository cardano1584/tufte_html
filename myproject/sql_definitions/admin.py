...
from .models import Sql_definitionsContent, Sql_definitionsPostVersion

class PostVersionInline(admin.TabularInline):
    model = Sql_definitionsPostVersion
    ...
    class Media:
        js = (
            '/static/sql_definitions/codemirror/lib/codemirror.js', 
            '/static/sql_definitions/codemirror/mode/markdown/markdown.js', 
            '/static/sql_definitions/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/sql_definitions/codemirror/lib/codemirror.css',
            )
        }
...
class Sql_definitionsContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            Sql_definitionsPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(Sql_definitionsPostVersion, pk=version_id)
...
admin.site.register(Sql_definitionsContent, Sql_definitionsContentAdmin)
