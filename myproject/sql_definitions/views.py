...
from .models import Sql_definitionsContent, Sql_definitionsPostViewTracking
...
def render_sql_definitions_post(request, slug):
    post = get_object_or_404(Sql_definitionsContent, slug=slug)
    if request.user.is_authenticated:
        Sql_definitionsPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'sql_definitions/sql_definitions_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = Sql_definitionsContent.objects.all()
    return render(request, 'sql_definitions/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_sql_definitions_post(request):
    if request.method == "POST":
        ...
        new_post = Sql_definitionsContent.objects.create(
            title=title,
            summary=summary,
