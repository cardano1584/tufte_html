...
from .models import ExtractionContent, ExtractionPostVersion

class PostVersionInline(admin.TabularInline):
    model = ExtractionPostVersion
    ...
    class Media:
        js = (
            '/static/extraction/codemirror/lib/codemirror.js', 
            '/static/extraction/codemirror/mode/markdown/markdown.js', 
            '/static/extraction/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/extraction/codemirror/lib/codemirror.css',
            )
        }
...
class ExtractionContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            ExtractionPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(ExtractionPostVersion, pk=version_id)
...
admin.site.register(ExtractionContent, ExtractionContentAdmin)
