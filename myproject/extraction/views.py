...
from .models import ExtractionContent, ExtractionPostViewTracking
...
def render_extraction_post(request, slug):
    post = get_object_or_404(ExtractionContent, slug=slug)
    if request.user.is_authenticated:
        ExtractionPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'extraction/extraction_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = ExtractionContent.objects.all()
    return render(request, 'extraction/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_extraction_post(request):
    if request.method == "POST":
        ...
        new_post = ExtractionContent.objects.create(
            title=title,
            summary=summary,
