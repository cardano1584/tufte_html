...
from .models import Sql_snippetsContent, Sql_snippetsPostViewTracking
...
def render_sql_snippets_post(request, slug):
    post = get_object_or_404(Sql_snippetsContent, slug=slug)
    if request.user.is_authenticated:
        Sql_snippetsPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'sql_snippets/sql_snippets_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = Sql_snippetsContent.objects.all()
    return render(request, 'sql_snippets/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_sql_snippets_post(request):
    if request.method == "POST":
        ...
        new_post = Sql_snippetsContent.objects.create(
            title=title,
            summary=summary,
