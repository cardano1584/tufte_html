...
from .models import Sql_snippetsContent, Sql_snippetsPostVersion

class PostVersionInline(admin.TabularInline):
    model = Sql_snippetsPostVersion
    ...
    class Media:
        js = (
            '/static/sql_snippets/codemirror/lib/codemirror.js', 
            '/static/sql_snippets/codemirror/mode/markdown/markdown.js', 
            '/static/sql_snippets/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/sql_snippets/codemirror/lib/codemirror.css',
            )
        }
...
class Sql_snippetsContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            Sql_snippetsPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(Sql_snippetsPostVersion, pk=version_id)
...
admin.site.register(Sql_snippetsContent, Sql_snippetsContentAdmin)
