...
from .models import SnippetsContent, SnippetsPostViewTracking
...
def render_snippets_post(request, slug):
    post = get_object_or_404(SnippetsContent, slug=slug)
    if request.user.is_authenticated:
        SnippetsPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'snippets/snippets_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = SnippetsContent.objects.all()
    return render(request, 'snippets/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_snippets_post(request):
    if request.method == "POST":
        ...
        new_post = SnippetsContent.objects.create(
            title=title,
            summary=summary,
