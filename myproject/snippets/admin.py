...
from .models import SnippetsContent, SnippetsPostVersion

class PostVersionInline(admin.TabularInline):
    model = SnippetsPostVersion
    ...
    class Media:
        js = (
            '/static/snippets/codemirror/lib/codemirror.js', 
            '/static/snippets/codemirror/mode/markdown/markdown.js', 
            '/static/snippets/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/snippets/codemirror/lib/codemirror.css',
            )
        }
...
class SnippetsContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            SnippetsPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(SnippetsPostVersion, pk=version_id)
...
admin.site.register(SnippetsContent, SnippetsContentAdmin)
