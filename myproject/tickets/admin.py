...
from .models import TicketsContent, TicketsPostVersion

class PostVersionInline(admin.TabularInline):
    model = TicketsPostVersion
    ...
    class Media:
        js = (
            '/static/tickets/codemirror/lib/codemirror.js', 
            '/static/tickets/codemirror/mode/markdown/markdown.js', 
            '/static/tickets/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/tickets/codemirror/lib/codemirror.css',
            )
        }
...
class TicketsContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            TicketsPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(TicketsPostVersion, pk=version_id)
...
admin.site.register(TicketsContent, TicketsContentAdmin)
