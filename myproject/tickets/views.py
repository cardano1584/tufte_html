...
from .models import TicketsContent, TicketsPostViewTracking
...
def render_tickets_post(request, slug):
    post = get_object_or_404(TicketsContent, slug=slug)
    if request.user.is_authenticated:
        TicketsPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'tickets/tickets_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = TicketsContent.objects.all()
    return render(request, 'tickets/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_tickets_post(request):
    if request.method == "POST":
        ...
        new_post = TicketsContent.objects.create(
            title=title,
            summary=summary,
