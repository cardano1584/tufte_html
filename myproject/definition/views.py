...
from .models import DefinitionContent, DefinitionPostViewTracking
...
def render_definition_post(request, slug):
    post = get_object_or_404(DefinitionContent, slug=slug)
    if request.user.is_authenticated:
        DefinitionPostViewTracking.objects.create(post=post, user=request.user)
    can_view_extra_info = request.user.groups.filter(name__in=['engineer', 'analyst']).exists()
    html_content = markdown.markdown(post.content)
    return render(request, 'definition/definition_post_view.html', {
        'post': post,
...
def sql_definitions_view(request):
    all_posts = DefinitionContent.objects.all()
    return render(request, 'definition/sql_definitions.html', {'all_posts': all_posts})

@csrf_exempt
def create_definition_post(request):
    if request.method == "POST":
        ...
        new_post = DefinitionContent.objects.create(
            title=title,
            summary=summary,
