...
from .models import DefinitionContent, DefinitionPostVersion

class PostVersionInline(admin.TabularInline):
    model = DefinitionPostVersion
    ...
    class Media:
        js = (
            '/static/definition/codemirror/lib/codemirror.js', 
            '/static/definition/codemirror/mode/markdown/markdown.js', 
            '/static/definition/js/init_codemirror.js', 
        )
        css = {
            'all': (
                '/static/definition/codemirror/lib/codemirror.css',
            )
        }
...
class DefinitionContentAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'content' in form.changed_data:
            DefinitionPostVersion.objects.create(
                post=obj, 
                title=obj.title, 
                summary=obj.summary, 
    ...
    def restore_version(self, request, version_id):
        version = get_object_or_404(DefinitionPostVersion, pk=version_id)
...
admin.site.register(DefinitionContent, DefinitionContentAdmin)
